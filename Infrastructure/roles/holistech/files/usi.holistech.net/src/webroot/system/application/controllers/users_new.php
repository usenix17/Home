...
class Users extends Controller
{
	...
	function index()
	{
		$this->load->view('control/control_user/search');
	}
	...
	function search()
	{
		...
		$sql = <<<SQL
			SELECT u.realName, u.user_id, u.email, u.phone, 
			...
			(SELECT max(l.time) FROM login_log l WHERE l.user_id=u.user_id) as last_login
			FROM users u
			LEFT JOIN purchases p ON p.user_id=u.user_id AND p.status!='Pending'
			WHERE 
				( u.realName LIKE ?
				OR u.username LIKE ?
				OR u.email LIKE ?
...
SQL;
		...
		if ( count($result) )
			$this->load->view('control/control_user/search_results',array('results'=>$result));
		else
	...
	/* Create the view */
		$this->load->view('control/control_user/details',array(
			'purchases' => $purchases,
	...
	/* Reload the view */
		$this->output->set_output('<SCRIPT>load("/control_user/show_details/'.$purchase['user_id'].'","#tech_support_details_'.$purchase['user_id'].'_TD");</SCRIPT>');
	}
	...
	/* Determine if any of the codes have been used to completion */
		...
		if ( $fail ) {
			$this->load->view('control/control_user/refund_fail',array('reasons'=>$reasons));
			return;
		}
	...
	/* Determine if any of the codes have been used at all */	
		...
		if ( $fail ) {
			$this->load->view('control/control_user/refund_warn',array(
				'reasons' => $reasons,
	...
	/* Verify the Address */	
		$this->load->view('control/control_user/address_verify',array(
				'codes' => $this->input->post('codes'),
				'user_id' => $user_id,
				'user' => $user,
				'destination' => '/control_user/refund_final',
		));
	}
	...
	/* Get the email body */
		$message = $this->load->view('control/control_user/refund_email',array(
			'purchases' => $purchases,
			'codes' => $codes,
			'user' => $user,
			'amount' => $amount,
		...
		$this->_email('Jason.Karcz@nau.edu',"az-hospitality.nau.edu refund request",$message);
	}
	...
	/* Verify the Address */	
		$this->load->view('control/control_user/address_verify',array(
				...
				'user_id' => $user_id,
				'user' => $user,
				'destination' => '/control_user/email_final',
		));
	}
	...
	/* Get the email body */
		$message = $this->load->view('control/control_user/codes_email',array(
			'codes' => $codes,
			'user' => $user,
		),TRUE);
	...
	/* Verify the Address */	
		$this->load->view('control/control_user/address_verify',array(
				...
				'user_id' => $user_id,
				'user' => $user,
				'destination' => '/control_user/certify_final/'.$enrollment_id,
		));
	}
