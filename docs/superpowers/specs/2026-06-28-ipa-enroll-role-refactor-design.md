...
### 1. Single source of IPA truth -- `inventory/group_vars/all/ipa.yml` (new)
...
- **Phase 1 -- shared vars (no behavior change):** add `inventory/group_vars/all/ipa.yml`; parameterize
...
**In:** `inventory/group_vars/all/ipa.yml`; the `ipa-enroll` role; parameterizing freebsd / omnios /
...
- An IPA server / realm / domain change is a single edit in `inventory/group_vars/all/ipa.yml`.
